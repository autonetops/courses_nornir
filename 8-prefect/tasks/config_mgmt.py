...
def configurar(task: Task, commit: bool = False) -> Result:
    ...
    render = task.run(task=template_file, template="base.j2", path=path, name="render")
    deploy = task.run(
...
def configurar_com_retry(
    ...
    for tentativa in range(1, tentativas):
        ...
        try:
            r = task.run(task=configurar, commit=commit, name=f"tentativa {tentativa}")
    ...
    r = task.run(task=configurar, commit=commit, name=f"tentativa {tentativas}")
    return Result(host=task.host, result=r.result, changed=r.changed)
